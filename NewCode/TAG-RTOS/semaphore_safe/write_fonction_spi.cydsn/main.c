...
uint8_t prio_matrice[NUMBER_OF_ANCHOR+1][NUMBER_OF_ANCHOR+1] = {{10,-10,-10,-10},
                                                                {-10,10,-10,-10},
                                                                {-10,-10,10,-10},
                                                                {-10,-10,-10,10},
                                                                }; //each anchor + usb_out + usb_in
#define ISR_PRIORITY                    (5u)
#define INTERRUPT_PRIORITY              (6u)
uint8_t prio1 = 20;
uint8_t prio2 = 20;
uint8_t prio3 = 20;
uint8_t prio4 = 20;
uint8_t usb_out_prio = 19;
...
static void change_priorities(int num){
    ...
    if(num == -1){
        ...
        vTaskPrioritySet(usbInHandler, usb_out_prio + 30);
...
static void state_machine_task(void *arg){	
    ...
    state = STATE_INIT;
	for(;;){
        if(xSemaphoreTake(semaphoreBuf1, 32) == pdPASS){
